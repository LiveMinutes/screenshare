  ...
  function diffEqual (a, b) {
    ...
    for (var y = 0; y < height; y++) {
	    for (var x = 0; x < width; x++) {
	        r = cData[i++] = Math.abs(aData[i] - bData[i]);
		    g = cData[i++] = Math.abs(aData[i+1] - bData[i+1]);
		    b = cData[i++] = Math.abs(aData[i+2] - bData[i+2]);
		    cData[i++] = Math.abs(255 - aData[i+3] - bData[i+3]); // alpha layer
